...
bool IsPathSafe(const string &root_dir, const string &test_file) {
  ...
  // STEP 1
  // Stores the absolute path of root_dir and test_file
  char resolved_root_path[PATH_MAX] = "";
  char resolved_test_path[PATH_MAX] = "";

  // Get the absolute paths for root_dir and test_file
  // realpath() resolves the paths, removing "." and "..",
  // and returns a normalized absolute path
  if (realpath(root_dir.c_str(), resolved_root_path) == nullptr ||
      realpath(test_file.c_str(), resolved_test_path) == nullptr) {
    return false;  // If path resolution fails, the file is considered unsafe
  }

  // Convert to std::string for easier manipulation
  string root_path_str(resolved_root_path);
  string test_path_str(resolved_test_path);

  // Ensure root_path_str ends with '/' to prevent incorrect matches
  // Example: "/home/user/test" should not match "/home/user/testing"
  if (!root_path_str.empty() && root_path_str.back() != '/') {
    root_path_str += '/';
  }

  // Use compare() to check if test_path_str starts with root_path_str
  // This ensures that test_file is within root_dir
  return test_path_str.compare(0, root_path_str.size(), root_path_str) == 0;
}
...
string EscapeHtml(const string &from) {
  ...
  // STEP 2
  // Iterate through the string and replace special HTML characters
  // with their corresponding escape sequences
  for (size_t i = 0; i < ret.length(); ++i) {
    switch (ret[i]) {
      case '<':
        ret.replace(i, 1, "&lt;");    // Replace '<' with "&lt;"
        break;
      case '>':
        ret.replace(i, 1, "&gt;");    // Replace '>' with "&gt;"
        break;
  ...
  return ret;  // Return the modified string with escaped characters
}
