...
static void GetPortAndPath(int argc,
  ...
  // Convert and validate the port number
  *port = atoi(argv[1]);
  ...
  *path = string(argv[2]);
  ...
  // Process and validate index files
  for (int i = 3; i < argc; i++) {
    ...
    // Add the validated index file to the list
    indices->push_back(file_path);
  }

  // Ensure at least one valid index file is provided
  if (indices->empty()) {
