...
bool FileReader::ReadFile(string *const contents) {
  ...
  char* buffer = nullptr;  // Buffer to hold file data
  int file_size = 0;  // Stores the size of the file

  // Read file contents into buffer
  // (uses malloc internally, requires manual free)
  buffer = ReadFileToString(full_file.c_str(), &file_size);
  if (!buffer) {
    cerr << "Error reading file: " << full_file << endl;
    return false;
  }

  // Assign the file content to the output string (ensuring binary correctness)
  *contents = string(buffer, file_size);
