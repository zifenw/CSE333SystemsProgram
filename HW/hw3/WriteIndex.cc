...
static int WriteHeader(FILE *f, int doctable_bytes, int memidx_bytes) {
  ...
  // invoke FoldByteIntoCRC() repeatedly, once for each byte in the sequence.
  for (int i = 0; i < doctable_bytes + memidx_bytes; i++) {
    uint8_t next_byte;
    // size_t fread(void *ptr, size_t size, size_t count, FILE *stream);
    if (fread(&next_byte, sizeof(next_byte), 1, f) != 1) {
      return kFailedWrite;
    }
...
static int WriteHashTable(FILE *f, IndexFileOffset_t offset, HashTable *ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    int chain_len = LinkedList_NumElements(ht->buckets[i]);
    int br_bytes = WriteHTBucketRecord(f, record_pos, chain_len, bucket_pos);

    // If the bucket's chain is not empty, write the bucket contents
    if (chain_len != 0) {
      int b_bytes = WriteHTBucket(f, bucket_pos, ht->buckets[i], fn);
      bucket_pos += b_bytes;  // update the bucket position
    }
    // update the bucket record position
    record_pos += br_bytes;
  }
...
static int WriteHTBucket(FILE *f, IndexFileOffset_t offset, LinkedList *li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    fseek(f, record_pos, SEEK_SET);
    // Create a element record that stores the byte offset of the element
    ElementPositionRecord record(element_pos);
    record.ToDiskFormat();

    // Write the record to the file. If fwrite() fails, return an error.
    ...
    // Retrieve the next key-value pair (payload) from the linked list.
    HTKeyValue_t *kv;
    LLIterator_Get(it, reinterpret_cast<LLPayload_t *>(&kv));

    // Use the function pointer to write the element data at element_pos
    ...
    element_pos += elt_byte;  // you may need to change this logic
    LLIterator_Next(it);
  }
  LLIterator_Free(it);
...
static int WriteDocidToDocnameFn(FILE *f, IndexFileOffset_t offset,
  ...
  if (fwrite(reinterpret_cast<char*>(kv->value), file_name_bytes, 1, f) != 1) {
    return kFailedWrite;
  }
...
static int WriteDocIDToPositionListFn(FILE *f,
  ...
  // Move file pointer to the designated offset before writing.
  if (fseek(f, offset, SEEK_SET) != 0) {
    return kFailedWrite;
  }
  ...
  int written_bytes = sizeof(header);
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    // Retrieve the next position from the linked list.
    LLPayload_t payload;
...
static int WriteWordToPostingsFn(FILE *f,
  ...
  // STEP 19.
  // Calculate and return the total amount of data written.
  return sizeof(header) + ht_bytes + word_bytes;
}
}  // namespace hw3
